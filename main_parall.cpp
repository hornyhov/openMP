...
int main() {
    ...
    while (norm > ACC || init_flag) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:norm_r) private(i, j) shared(a, b, F, r, w, M, N, h1, h2)
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:norm_dr) private(i, j) shared(a, b, r, Ar, M, N, h1, h2)
        for (i = 1; i < M-1; i++) {
            for (j = 1; j < N-1; j++) {
        ...
        #pragma omp parallel for reduction(+:norm) private(i, j, tmp) shared(w, r, Diff, M, N, tau, h1, h2)
        for (i = 1; i < M; i++) {
            for (j = 1; j < N; j++) {
                tmp = w[i][j];
