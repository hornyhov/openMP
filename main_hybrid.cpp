...
int main(int argc, char **argv){
    ...
    while (global_norm > ACC || init_flag) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:norm_r) private(i, j) shared(a, b, F, r, row_start, col_start, row_end, col_end, w, M, N, h1, h2)
        ...
        #pragma omp parallel for collapse(2) reduction(+:norm_dr) private(i, j) shared(a, b, r, Ar, M, N, row_start, col_start, row_end, col_end, h1, h2)
        for (i = row_start + 1; i < row_end; i++) {
            for (j = col_start + 1; j < col_end; j++) {
                if(i == 0 || j == 0 || i == M-1 || j == N-1){
                    continue;
